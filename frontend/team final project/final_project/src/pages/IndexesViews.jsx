...
export default function IndexesViews() {
  const navigate = useNavigate();
  const color = '#14b8a6'; // Teal for performance & structured views
  const glow = 'rgba(20, 184, 166, 0.15)';

  const sections = [
    {
      num: '01', name: 'INDEX', tagline: 'Speed up query retrievals',
      definition: 'An Index is a physical structure in a database that speeds up record lookup operations on specific columns. However, they slow down writes (INSERT, UPDATE, DELETE).',
      syntax: `<span class="kw">CREATE INDEX</span> index_name\n<span class="kw">ON</span> <span class="tbl">table_name</span> (<span class="col">column1</span>, ...);`,
      examples: [
        {
          label: 'Create index on student email field',
          code: `<span class="kw">CREATE INDEX</span> idx_students_email\n<span class="kw">ON</span> <span class="tbl">Students</span> (<span class="col">email</span>);\n<span class="cmt">-- Speeds up queries like WHERE email = '...'</span>`,
        }
      ],
      usecase: 'Created on columns frequently referenced in WHERE, JOIN, and ORDER BY clauses in high-traffic applications.',
    },
    {
      num: '02', name: 'VIEW', tagline: 'Virtual Tables',
      definition: 'A View is a virtual table representing the output of a pre-defined SELECT query. Views do not store physical data themselves, but dynamically query real tables.',
      syntax: `<span class="kw">CREATE VIEW</span> view_name <span class="kw">AS</span>\n<span class="kw">SELECT</span> ...;`,
      examples: [
        {
          label: 'Create a view containing student contact cards',
          code: `<span class="kw">CREATE VIEW</span> StudentContactCards <span class="kw">AS</span>\n<span class="kw">SELECT</span> <span class="col">student_id</span>, <span class="col">full_name</span>, <span class="col">email</span>, <span class="col">phone</span>\n<span class="kw">FROM</span> <span class="tbl">Students</span>\n<span class="kw">WHERE</span> <span class="col">phone</span> <span class="kw">IS NOT NULL</span>;`,
        }
      ],
      usecase: 'Used to simplify complex joins, restrict user access to sensitive columns (hiding password columns), and establish clear data access interfaces.',
    },
  ];
      ...
      <div className="concept-page page-enter">
        <div className="cp-hero" style={{ background: `linear-gradient(135deg, var(--bg-primary) 0%, ${glow} 100%)` }}>
          <div className="cp-hero-inner">
            ...
            <div className="cp-badge" style={{ background: glow, color }}>
              ⚡ Indexes & Views — Optimization
            </div>
            <h1 className="cp-title">
              Indexes & <span style={{ color }}>Views</span>
            </h1>
            <p className="cp-subtitle">
              Master how to speed up database query times using Indexes, and simplify reporting logic using Views.
            </p>
          </div>
        </div>
        ...
        <div className="cp-content">
          <div className="toc-strip">
            {sections.map(c => (
              <a key={c.name} href={`#idxview-${c.name}`} className="toc-link">{c.name}</a>
            ))}
          </div>

          {sections.map((cmd) => (
            <div key={cmd.name} id={`idxview-${cmd.name}`} className="cmd-section">
          ...
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', marginTop: '24px' }}>
            <button className="btn-primary" onClick={() => navigate('/aggregate')} id="go-aggregate-btn">
